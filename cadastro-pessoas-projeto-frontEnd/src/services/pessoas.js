...
export default (httpClient) => ({
  getPessoas: async () => {
    ...
    response.data.forEach((pessoa) => {
      let dadosPessoa = reactive({
        ...
        categorias: pessoa.categorias_interesse?.map((cat) => ({
          nome: cat.nome_categoria ? cat.nome_categoria : "teste",
        })),
      });
  ...
  updatePessoa: async (pessoa, id) => {
    console.log(pessoa, id);
    const response = await httpClient.put(`/atualizar/${id}`, pessoa);
    return {
      data: response.data,
    };
  },
